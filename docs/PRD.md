...
- **Real-time streaming transcript:** the current pipeline waits for a VAD-detected pause (400ms silence) before transcribing a whole closed segment with Whisper — this reads as slow and chunky compared to something like Google Meet's live captions, and fast speech in particular doesn't transcribe accurately under the current batch-per-segment approach. **Feasibility spike done Day 16, implemented and confirmed working Day 17 (2026-09-04). Reverted Day 18 (2026-09-04/05)** after real live testing kept surfacing the same root cause under different symptoms — decoding a partial/boundary-aligned buffer slice, rather than a complete pause-bounded utterance, is inherently fragile regardless of Whisper model size, across several distinct failure modes (silence hallucination, a short-context re-decode scoring worse than the original, a same-length-but-wrong substitution, a word duplicated at a segment boundary between two independently-decoded models). Back to this app's original pause-then-transcribe-the-whole-utterance architecture, on `small.en` (this app's pre-Day-17 default) — real testing confirmed materially better accuracy with no hallucination artifacts, at the cost of no live partial captions (text appears once per finished sentence, not word-by-word). The hallucination-defense filters discovered along the way (silence-amplitude gate, `no_speech_prob`/`avg_logprob` segment filtering) were kept and now guard the segment-based flow too. Full account in `project_notes.md` (gitignored, local only) and `DEV_PLAN.md`, Day 18.
- **Suggestion streaming (Day 18, 2026-09-05):** a suggestion could take 7-12s to generate in full for the fuller Interview-mode format (see below), with nothing shown the whole time. The `claude` CLI's `--include-partial-messages` flag turns out to expose real token-level streaming (confirmed live, not assumed); wired through so windows_app now receives and displays the answer as it's generated, rather than waiting for the complete text — unchanged total generation time, but the perceived wait dropped from ~7-12s to ~1-3s until something is visible.

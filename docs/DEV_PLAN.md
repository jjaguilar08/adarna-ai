...
## Day 18.10 — Live-Agent-Listening: Real-Duration Unattended Test 🔧 prep done, Jon's manual run still pending
...
**Prep done, 2026-09-06:** `wsl_app/research/day18_10/live_listen_harness_long.py` built. Shape chosen: the 5 existing synthetic test clips (day16/day18_5/day18_6, no new audio recorded) played back-to-back as one ~193s cycle, repeated 9 times, with 3 deliberate 5-minute silent stretches inserted after cycles 3/6/9 — ~44 real minutes total, ~34% of it silence. Unlike Day 18.8's harness (which only ever printed one hand-scripted `TRIGGER` line at the very end), this one runs a real `asyncio` event loop against the actual, unmodified `main.SuggestionTrigger` class, so `TRIGGER` lines fire from real pause-timer logic the same way production does — needed for the turn-count estimate to be real rather than guessed. Also adds a 60-second heartbeat (so a deliberate idle stretch doesn't read identically to a stall after the fact), per-cycle summary lines (segment/trigger counts and average transcribe latency, cycle 1 directly comparable to cycle 9 for the degradation question), and a final summary block with total notification count and first-10-vs-last-10 latency comparison. Verified with a `--smoke-test` flag (~70s shrunken run: 2 clips, 1 cycle, a 5s idle stretch) before trusting it with the full 44-minute run — exit 0, 6/6 segments and triggers fired correctly (matching Day 18.8's own ~7-turns-per-46s-clip finding almost exactly), heartbeat and cycle/summary lines all printed correctly, zero errors. Full writeup: `wsl_app/research/day18_10/README.md`. **Not run yet** — the real 30-60 minute test is Jon's manual step, unchanged from the plan above.

## Day 19 — Mic Capture (Phase 3 item, pulled forward)
...
## Day 20 — Opt-in Session Persistence (Phase 2, part 3)

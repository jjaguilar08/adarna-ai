...
**What's confirmed vs. still open:** the suggestion-format fix (bug 3) is live-verified against the real CLI with varied inputs — the strongest evidence this project's process produces short of Jon's own hands. Bugs 1 and 2 (mic VAD strictness, transcribe concurrency lock) are code-reviewed and offline-logic-tested but need a real mic session to confirm they actually fixed what Jon saw, since neither can be exercised without real mic hardware and real background noise. Also still unconfirmed for the same reason as before: whether the transcribe_lock's serialization (by design) meaningfully changes the *felt* latency when both sources are genuinely talking at once — expected to be a net win per Day 18.6's contention finding, but not yet timed on a real mixed-source session.
